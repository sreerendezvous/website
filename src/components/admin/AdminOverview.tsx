...
import { Users, Star, FileCheck, ArrowUp, ArrowDown } from 'lucide-react';
...
export function AdminOverviewPage() {
  ...
  // Calculate counts
  const creatorCount = users?.filter(user => user.role === 'creator').length || 0;
  const pendingExperiencesCount = experiences?.filter(exp => exp.booking_type === 'request').length || 0;
            ...
            {(adminActions || []).slice(0, 5).map((action) => (
              <div key={action.id} className="flex items-start gap-4">
                <div className={`h-8 w-8 rounded-full flex items-center justify-center ${
                  action.action_type === 'update_role' ? 'bg-green-500/20 text-green-400' :
                  action.action_type === 'delete' ? 'bg-red-500/20 text-red-400' :
                  'bg-blue-500/20 text-blue-400'
                }`}>
                  {action.action_type === 'update_role' ? <FileCheck className="h-4 w-4" /> :
                   action.action_type === 'delete' ? <Star className="h-4 w-4" /> :
                ...
                <div>
                  <p className="text-sand-100">
                    ...
                    {' '}
                    {action.action_type === 'update_role' ? 'updated role for' :
                     action.action_type === 'delete' ? 'deleted' :
                     'modified'} a {action.target_type}
                  </p>
