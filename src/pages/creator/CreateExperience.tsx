...
export function CreateExperience() {
  ...
  const { register, handleSubmit, setValue, watch, formState: { errors, isSubmitting } } = useForm<ExperienceFormData>({
  ...
  const onSubmit = async (data: ExperienceFormData) => {
    try {
      ...
      const mediaPromises = data.media.map(async (media, index) => {
        // If it's already a URL (from previous upload), use it as is
      ...
      // Create the experience with the correct media structure
      await experienceService.createExperience(user.id, {
        ...data,
        media: uploadedMedia
      });

      // Navigate to the experiences list
  ...
  if (!user || user.role !== 'creator') {
    return (
      <div className="min-h-screen pt-20 flex items-center justify-center">
        <p className="text-sand-400">You must be an approved creator to create experiences.</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen pt-20 pb-12 bg-earth-900">
      <div className="container mx-auto px-4">
        <div className="max-w-3xl mx-auto">
          <h1 className="text-3xl font-display mb-8">Create New Experience</h1>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 bg-earth-800/50 p-6 rounded-lg">
            ...
            <div className="flex justify-end space-x-4">
              <Button
                type="button"
                variant="outline"
                onClick={() => navigate('/creator/experiences')}
              >
                Cancel
              </Button>
